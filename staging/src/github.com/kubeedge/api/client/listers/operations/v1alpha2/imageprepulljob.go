...
type ImagePrePullJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.ImagePrePullJob, err error)
	// Get retrieves the ImagePrePullJob from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.ImagePrePullJob, error)
...
// List lists all ImagePrePullJobs in the indexer.
func (s *imagePrePullJobLister) List(selector labels.Selector) (ret []*v1alpha2.ImagePrePullJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.ImagePrePullJob))
	})
	return ret, err
}

// Get retrieves the ImagePrePullJob from the index for a given name.
func (s *imagePrePullJobLister) Get(name string) (*v1alpha2.ImagePrePullJob, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("imageprepulljob"), name)
	}
	return obj.(*v1alpha2.ImagePrePullJob), nil
}
