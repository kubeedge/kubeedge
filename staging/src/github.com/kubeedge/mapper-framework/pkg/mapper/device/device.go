...
import (
	...
	dbInflux "github.com/kubeedge/mapper-framework/pkg/mapper/data/dbmethod/influxdb2"
	dbMysql "github.com/kubeedge/mapper-framework/pkg/mapper/data/dbmethod/mysql"
	dbRedis "github.com/kubeedge/mapper-framework/pkg/mapper/data/dbmethod/redis"
	dbTdengine "github.com/kubeedge/mapper-framework/pkg/mapper/data/dbmethod/tdengine"
	httpMethod "github.com/kubeedge/mapper-framework/pkg/mapper/data/publish/http"
	mqttMethod "github.com/kubeedge/mapper-framework/pkg/mapper/data/publish/mqtt"
	otelMethod "github.com/kubeedge/mapper-framework/pkg/mapper/data/publish/otel"
	"github.com/kubeedge/mapper-framework/pkg/mapper/data/stream"
	"github.com/kubeedge/mapper-framework/pkg/driver"
...
type DevPanel struct {
	adapterFactory func(protocol []byte) (driver.Adapter, error)
	deviceMuxs     map[string]context.CancelFunc
	devices        map[string]*CustomizedDev
...
func NewDevPanel(factory func(protocol []byte) (driver.Adapter, error)) *DevPanel {
	once.Do(func() {
		devPanel = &DevPanel{
			adapterFactory: factory,
			deviceMuxs:     make(map[string]context.CancelFunc),
			devices:        make(map[string]*CustomizedDev),
...
// start the device
func (d *DevPanel) start(ctx context.Context, dev *CustomizedDev) {
	defer d.wg.Done()

	client, err := d.adapterFactory(dev.Instance.PProtocol.ConfigData)
	if err != nil {
		klog.Errorf("Init dev %s error: %v", dev.Instance.Name, err)
		return
	}
	dev.Adapter = client
	err = dev.Adapter.InitDevice()
	if err != nil {
...
func dataHandler(ctx context.Context, dev *CustomizedDev) {
	...
	for _, twin := range dev.Instance.Twins {
		...
		if twin.Property.PProperty.DataType == "stream" {
			err = stream.StreamHandler(&twin, dev.Adapter, &visitorConfig)
			if err != nil {
		...
		if twin.Property.PushMethod.MethodConfig != nil && twin.Property.PushMethod.MethodName != "" {
			pushHandler(ctx, &twin, dev.Adapter, &visitorConfig, dataModel)
		}
		// handle anomaly detection
		if twin.Property.PushMethod.AnomalyDetectionConfig != nil && twin.Property.PushMethod.AnomalyDetectionEnabled {
			adHandler(ctx, &twin, dev.Adapter, &visitorConfig)
		}
		// handle database
		if twin.Property.PushMethod.DBMethod.DBMethodName != "" {
			dbHandler(ctx, &twin, dev.Adapter, &visitorConfig, dataModel)
		}
	}
}

// pushHandler start data panel work
func pushHandler(ctx context.Context, twin *common.Twin, client driver.Adapter, visitorConfig *driver.VisitorConfig, dataModel *common.DataModel) {
...
func dbHandler(ctx context.Context, twin *common.Twin, client driver.Adapter, visitorConfig *driver.VisitorConfig, dataModel *common.DataModel) {
...
func adHandler(ctx context.Context, twin *common.Twin, client driver.Adapter, visitorConfig *driver.VisitorConfig) {
...
func setVisitor(visitorConfig *driver.VisitorConfig, twin *common.Twin, dev *CustomizedDev) error {
	...
	err := dev.Adapter.SetDeviceData(value, visitorConfig)
	if err != nil {
...
func (d *DevPanel) UpdateDev(model *common.DeviceModel, device *common.DeviceInstance) {
	...
	// start new device
	d.devices[device.ID] = new(CustomizedDev)
...
func getTwinData(deviceID string, twin common.Twin, dev *CustomizedDev) ([]byte, error) {
	var visitorConfig driver.VisitorConfig
	err := json.Unmarshal(twin.Property.Visitors, &visitorConfig)
	if err != nil {
		return nil, err
...
func (d *DevPanel) WriteDevice(deviceMethodName, deviceID, propertyName, data string) error {
	...
	err = dev.Adapter.DeviceDataWrite(&visitorConfig, deviceMethodName, propertyName, writeData)
	if err != nil {
		return err
	}
	return nil
...
func (d *DevPanel) GetTwinResult(deviceID string, twinName string) (string, string, error) {
	...
	for _, twin := range dev.Instance.Twins {
		...
		data, err := dev.Adapter.GetDeviceData(&visitorConfig)
		if err != nil {
