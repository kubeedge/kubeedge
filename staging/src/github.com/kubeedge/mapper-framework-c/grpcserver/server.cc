...
static int write_modbus_direct_for_device(const std::string &deviceKey, const std::string &prop, const std::string &val);
                                ...
                                ::v1beta1::UpdateDeviceResponse *response) override
    {
        ...
        if (force && std::strcmp(force, "1") == 0 && dev.has_spec())
        {
            ...
            for (int i = 0; i < spec.properties_size(); ++i)
            {
                ...
                int wrc = write_modbus_direct_for_device(key, propName, desired);
        ...
        if (arc != 0 && dev.has_spec())
        {
            ...
            for (int i = 0; i < spec.properties_size(); ++i)
            {
                ...
                if (desired.empty())
                    continue;
                int wrc = write_modbus_direct_for_device(dev.namespace_().empty() ? dev.name() : (dev.namespace_() + "/" + dev.name()), propName, desired);
...
static int write_modbus_direct_for_device(const std::string &deviceKey, const std::string &prop, const std::string &val)
{
    Device *local = nullptr;
    if (g_device_manager)
    {
        local = device_manager_get(g_device_manager, deviceKey.c_str());
        if (!local)
        {
            const char *sep = strrchr(deviceKey.c_str(), '/');
            if (!sep)
                sep = strrchr(deviceKey.c_str(), '.');
            if (sep && *(sep + 1))
            {
                std::string shortId = std::string(sep + 1);
                local = device_manager_get(g_device_manager, shortId.c_str());
            }
        }
    }
    ...
    if (local)
    {
        offset = device_resolve_offset(local, prop.c_str());
    }
    if (offset <= 0)
    {
        log_warn("DirectWrite skip: unknown prop %s for deviceKey=%s", prop.c_str(), deviceKey.c_str());
        return -1;
    }
    ...
    log_info("DirectWrite exec: %s (device=%s)", cmd, deviceKey.c_str());
    int rc = std::system(cmd);
    log_info("DirectWrite rc=%d (device=%s prop=%s val=%s)", rc, deviceKey.c_str(), prop.c_str(), val.c_str());
    if (rc != 0)
    {
        log_error("DirectWrite failed rc=%d to %s:%d HR[%d]=%d (device=%s)", rc, host.c_str(), port, offset, iv, deviceKey.c_str());
        return -1;
    }
    log_info("DirectWrite OK to %s:%d HR[%d]=%d (device=%s prop=%s)", host.c_str(), port, offset, iv, deviceKey.c_str(), prop.c_str());
    return 0;
}
    ...
    if (!mgr)
    {
        ...
        if (dev.has_spec())
        {
            ...
            for (int i = 0; i < spec.properties_size(); ++i)
            {
                ...
                if (desired.empty())
                    continue;
                ok += (write_modbus_direct_for_device(dev.namespace_().empty() ? dev.name() : (dev.namespace_() + "/" + dev.name()), propName, desired) == 0);
            }
        }
        return ok > 0 ? 0 : -1;
    }
