...
-----END EC PRIVATE KEY-----`)
	...
	t.Run("TLS enabled", func(t *testing.T) {
		...
		if opts == nil {
			t.Fatalf("HubClientInit returned nil with valid certificate")
		}
		if opts.TLSConfig == nil {
			t.Fatalf("TLS config is nil")
		}
		if opts.TLSConfig.InsecureSkipVerify {
			t.Errorf("InsecureSkipVerify should be false")
		}
	...
	t.Run("Invalid CA file", func(t *testing.T) {
		...
		opts := HubClientInit("tcp://127.0.0.1:1883", "testClient", "", "")
		if opts != nil {
			t.Errorf("Expected nil result with invalid CA")
		}
	})
	...
	t.Run("TLS disabled", func(t *testing.T) {
		...
		if opts == nil {
			t.Errorf("Expected non-nil result with TLS disabled")
		} else if opts.TLSConfig == nil {
			t.Errorf("TLS config should not be nil")
		} else if !opts.TLSConfig.InsecureSkipVerify {
			t.Errorf("InsecureSkipVerify should be true when TLS is disabled")
		}
	})
}
