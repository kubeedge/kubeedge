...
func dealTwinSync(context *dtcontext.DTContext, resource string, msg interface{}) error {
	...
	if err != nil {
		...
		if err2 := dealUpdateResult(context, "", "", dtcommon.BadRequestCode,
			errors.New("unmarshal update request body failed, Please check the request"), result); err2 != nil {
	...
	if err := DealDeviceTwin(context, resource, eventID, msgTwin.Twin, SyncDealType); err != nil {
		klog.Errorf("DealDeviceTwin failed: %v", err)
...
func dealTwinGet(context *dtcontext.DTContext, resource string, msg interface{}) error {
	...
	if err := DealGetTwin(context, resource, content); err != nil {
		klog.Errorf("DealGetTwin failed: %v", err)
		return err
	}
	return nil
}
...
func Updated(context *dtcontext.DTContext, deviceID string, payload []byte) error {
	...
	if err != nil {
		klog.Errorf("Unmarshal update request body failed, err: %#v", err)
		if err2 := dealUpdateResult(context, "", "", dtcommon.BadRequestCode, err, result); err2 != nil {
			klog.Errorf("dealUpdateResult failed: %v", err2)
		}
		return err
	}
...
func DealDeviceTwin(context *dtcontext.DTContext, deviceID string, eventID string, msgTwin map[string]*dttype.MsgTwin, dealType int) error {
	...
	if !isExist {
		...
		if err2 := dealUpdateResult(context, deviceID, eventID, dtcommon.NotFoundCode,
			errors.New("update rejected due to the device is not existed"), result); err2 != nil {
			klog.Errorf("dealUpdateResult failed: %v", err2)
		}
	...
	if content == nil {
		...
		if err2 := dealUpdateResult(context, deviceID, eventID, dtcommon.BadRequestCode, err, result); err2 != nil {
			klog.Errorf("dealUpdateResult failed: %v", err2)
		}
		return err
	}
	...
	if dealType == RestDealType && dealTwinResult.Err != nil {
		if err2 := SyncDeviceFromSqlite(context, deviceID); err2 != nil {
		...
		if err2 := dealUpdateResult(context, deviceID, eventID, dtcommon.BadRequestCode, err, updateResult); err2 != nil {
			klog.Errorf("dealUpdateResult failed: %v", err2)
		}
		return err
	}
	...
	if len(dealTwinResult.Document) > 0 {
		if err := dealDocument(context, deviceID, dttype.BaseMessage{EventID: eventID, Timestamp: now}, dealTwinResult.Document); err != nil {
			klog.Errorf("dealDocument failed: %v", err)
		}
	}
	...
	if ok {
		if err := dealDelta(context, deviceID, delta); err != nil {
			klog.Errorf("dealDelta failed: %v", err)
		}
	}
	...
	if len(dealTwinResult.SyncResult) > 0 {
		if err := dealSyncResult(context, deviceID, dttype.BuildBaseMessage(), dealTwinResult.SyncResult); err != nil {
			klog.Errorf("dealSyncResult failed: %v", err)
		}
	}
	return nil
}
