...
Requires the update-status parameter.`)
		...
		annotationsPrefix = flags.String("annotations-prefix", controller.DefaultAnnotationsPrefix,
...
Takes the form "<host>:port". If not provided, no admission controller is started.`)
		...
		statusUpdateInterval = flags.Int("status-update-interval", controller.UpdateInterval, "Time interval in seconds in which the status should check if an update is required. Default is 60 seconds")
	)

	flags.StringVar(&controller.MaxmindLicenseKey, "maxmind-license-key", "", `Maxmind license key to download GeoLite2 Databases.
https://blog.maxmind.com/2019/12/18/significant-changes-to-accessing-and-using-geolite2-databases`)
	flags.StringVar(&controller.MaxmindEditionIDs, "maxmind-edition-ids", "GeoLite2-City,GeoLite2-ASN", `Maxmind edition ids to download GeoLite2 Databases.`)
	...
	if *statusUpdateInterval < 5 {
		...
		controller.UpdateInterval = *statusUpdateInterval
	}

	if *ingressClass != "" {
		klog.Infof("Watching for Ingress", "class", *ingressClass)

		if *ingressClass != controller.DefaultClass {
	...
	controller.AnnotationsPrefix = *annotationsPrefix
	...
	controller.StatusPort = *statusPort
	controller.StreamPort = *streamPort
	controller.ProfilerPort = *profilerPort
	...
	controller.HealthPath = *defHealthzURL

	if *defHealthCheckTimeout > 0 {
		controller.HealthCheckTimeout = time.Duration(*defHealthCheckTimeout) * time.Second
	}
	...
	//if MaxmindLicenseKey != "" && MaxmindEditionIDs != "" {
	//	if err := ValidateGeoLite2DBEditions(); err != nil {
	//		return false, nil, err
	//	}
	//	klog.Infof("downloading maxmind GeoIP2 databases")
	//	if err := DownloadGeoLite2DB(); err != nil {
	//		klog.Errorf(err, "unexpected error downloading GeoIP2 database")
	//	}
	//	config.MaxmindEditionFiles = MaxmindEditionFiles
	//}
