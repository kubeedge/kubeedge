...
func (n *NginxController) CheckIngress(ing *networking.Ingress) error {
	...
	defaults.SetDefaultNginxPathType(ing)
	//
...
func (n *NginxController) syncIngress(interface{}) error {
	...
	if !n.IsDynamicConfigurationEnough(pcfg) {
		...
		if err != nil {
			...
			n.recorder.Eventf(IngressPodDetails, apiv1.EventTypeWarning, "RELOAD", fmt.Sprintf("Error reloading NGINX: %v", err))
			return err
		}
		...
		n.recorder.Eventf(IngressPodDetails, apiv1.EventTypeNormal, "RELOAD", "NGINX reload triggered due to a change in configuration")
	}
...
func (n *NginxController) getBackendServers(ingresses []*ingress.Ingress) ([]*ingress.Backend, []*ingress.Server) {
	...
	for _, ing := range ingresses {
		ingKey := defaults.MetaNamespaceKey(ing)
...
func (n *NginxController) getStreamServices(configmapName string, proto apiv1.Protocol) []ingress.L4Service {
	...
	_, _, err := ParseNameNS(configmapName)
	if err != nil {
	...
	for port, svcRef := range configmap.Data {
		...
		svcNs, svcName, err := ParseNameNS(nsName)
		if err != nil {
			klog.Warningf("%v", err)
			continue
		}
	...
	for _, rule := range ing.Spec.Rules {
		...
		for _, path := range rule.HTTP.Paths {
			...
			for _, existing := range existingIngresses {
				...
				if annotationErr == defaults.ErrMissingAnnotations && existingAnnotationErr == existingAnnotationErr {
					return fmt.Errorf(`host "%s" and path "%s" is already defined in ingress %s/%s`, rule.Host, path.Path, existing.Namespace, existing.Name)
				}
			}
