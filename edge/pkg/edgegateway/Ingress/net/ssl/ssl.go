...
func getFakeHostSSLCert(host string) ([]byte, []byte) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			...
			CommonName:   "KubeEdge Ingress Controller Fake Certificate",
		},
		NotBefore: notBefore,
		NotAfter:  notAfter,
...
func StoreSSLCertOnDisk(name string, sslCert *ingress.SSLCert) (string, error) {
	...
	err := ioutil.WriteFile(pemFileName, []byte(sslCert.PemCertKey), controller.ReadWriteByUser)
	if err != nil {
...
func getPemFileName(fullSecretName string) (string, string) {
	...
	return fmt.Sprintf("%v/%v", controller.DefaultSSLDirectory, pemName), pemName
}
...
func NewTLSListener(certificate, key string) *TLSListener {
	...
	_, _ = defaults.NewFileWatcher(certificate, l.load)
	_, _ = defaults.NewFileWatcher(key, l.load)
...
func ConfigureCRL(name string, crl []byte, sslCert *ingress.SSLCert) error {
	...
	crlFileName := fmt.Sprintf("%v/%v", controller.DefaultSSLDirectory, crlName)
...
func ConfigureCACert(name string, ca []byte, sslCert *ingress.SSLCert) error {
	...
	fileName := fmt.Sprintf("%v/%v", controller.DefaultSSLDirectory, caName)
