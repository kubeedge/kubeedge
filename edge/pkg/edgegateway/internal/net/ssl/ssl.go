...
func getFakeHostSSLCert(host string) ([]byte, []byte) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			...
			CommonName:   "KubeEdge internal Controller Fake Certificate",
		},
		NotBefore: notBefore,
		NotAfter:  notAfter,
...
func CreateSSLCert(cert, key []byte, uid string) (*ingress.SSLCert, error) {
	...
	if ngx_config.EnableSSLChainCompletion {
		...
		if err != nil {
			klog.Error(err, "Error generating certificate chain for Secret")
		} else {
			pemCertBuffer.Reset()
	...
	if len(pemCert.Extensions) > 0 {
		klog.V(3).Infof("parsing ssl certificate extensions")
...
func StoreSSLCertOnDisk(name string, sslCert *ingress.SSLCert) (string, error) {
	...
	err := ioutil.WriteFile(pemFileName, []byte(sslCert.PemCertKey), edgegateway.ReadWriteByUser)
	if err != nil {
...
func getPemFileName(fullSecretName string) (string, string) {
	...
	return fmt.Sprintf("%v/%v", edgegateway.DefaultSSLDirectory, pemName), pemName
}
...
func SHA1(filename string) string {
	...
	if err != nil {
		klog.Error(err, "Error reading file", "path", filename)
		return ""
	}
...
func (tl *TLSListener) load() {
	klog.Info("loading tls certificate", "path", tl.certificatePath, "key", tl.keyPath)
...
func ConfigureCRL(name string, crl []byte, sslCert *ingress.SSLCert) error {
	...
	crlFileName := fmt.Sprintf("%v/%v", edgegateway.DefaultSSLDirectory, crlName)
...
func ConfigureCACert(name string, ca []byte, sslCert *ingress.SSLCert) error {
	...
	fileName := fmt.Sprintf("%v/%v", edgegateway.DefaultSSLDirectory, caName)
	...
	klog.V(3).Info("Created CA Certificate for Authentication", "path", fileName)
...
func AddOrUpdateDHParam(name string, dh []byte) (string, error) {
	...
	tempPemFile, err := ioutil.TempFile(edgegateway.DefaultSSLDirectory, pemName)

	klog.V(3).Info("Creating temporal file for DH", "path", tempPemFile.Name(), "name", pemName)
	if err != nil {
