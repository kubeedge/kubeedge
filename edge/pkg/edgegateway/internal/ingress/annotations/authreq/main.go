...
import (
	...
	"github.com/kubeedge/kubeedge/edge/pkg/edgegateway/internal/sets"
...
// Config returns external authentication configuration for an internal rule
type Config struct {
	URL string `json:"url"`
...
func (a authReq) Parse(ing *networking.Ingress) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.InvalidContent{Name: err.Error()}
	}

	authMethod, _ := parser.GetStringAnnotation("auth-method", ing)
	if len(authMethod) != 0 && !ValidMethod(authMethod) {
		return nil, errors.NewLocationDenied("invalid HTTP method")
	}
		...
		for _, header := range harr {
			...
			if len(header) > 0 {
				if !ValidHeader(header) {
					return nil, errors.NewLocationDenied("invalid headers list")
				}
	...
	if proxySetHeaderMap != "" {
		...
		if err != nil {
			return nil, errors.NewLocationDenied(fmt.Sprintf("unable to find configMap %q", proxySetHeaderMap))
		}

		for header := range proxySetHeadersMapContents.Data {
			if !ValidHeader(header) {
				return nil, errors.NewLocationDenied("invalid proxy-set-headers in configmap")
			}
		}
		...
		for _, duration := range arr {
			...
			if len(duration) > 0 {
				if !ValidCacheDuration(duration) {
					...
					return authCacheDuration, errors.NewLocationDenied(fmt.Sprintf("invalid cache duration: %s", duration))
				}
