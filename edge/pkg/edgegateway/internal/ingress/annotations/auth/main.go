...
// Config returns authentication configuration for an internal rule
type Config struct {
...
func (a auth) Parse(ing *networking.Ingress) (interface{}, error) {
	...
	if !authTypeRegex.MatchString(at) {
		return nil, errors2.NewLocationDenied("invalid authentication type")
	}
	...
	if err != nil {
		return nil, errors2.LocationDenied{
...
func dumpSecretAuthFile(filename string, secret *api.Secret) error {
	...
	if !ok {
		return errors2.LocationDenied{
			Reason: errors.Errorf("the secret %v does not contain a key with value auth", secret.Name),
		}
	}

	err := ioutil.WriteFile(filename, val, controller.ReadWriteByUser)
	if err != nil {
		return errors2.LocationDenied{
...
func dumpSecretAuthMap(filename string, secret *api.Secret) error {
	...
	err := ioutil.WriteFile(filename, []byte(builder.String()), controller.ReadWriteByUser)
	if err != nil {
		return errors2.LocationDenied{
			Reason: errors.Wrap(err, "unexpected error creating password file"),
		}
