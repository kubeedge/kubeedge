...
// statusSync keeps the status IP in each internal rule updated executing a periodic check
// in all the defined rules. To simplify the process leader election is used so the update
// is executed only in one node (internal controllers can be scaled to more than one)
...
func (s statusSync) Run(stopCh chan struct{}) {
	...
	// an item to trigger the update of the internal status.
...
func (s statusSync) Shutdown() {
	...
	if !s.UpdateStatusOnShutdown {
		klog.Warningf("skipping update of status of internal rules")
		return
	}
	...
	if s.isRunningMultiplePods() {
		klog.V(2).InfoS("skipping internal status update (multiple pods running - another one will be elected as master)")
		return
	}
...
func (s *statusSync) sync(key interface{}) error {
	if s.syncQueue.IsShuttingDown() {
		klog.V(2).InfoS("skipping internal status update (shutting down in progress)")
		return nil
	}

	_, err := s.runningAddresses()
	if err != nil {
		return err
	}
...
// updateStatus changes the status information of internal rules
func (s *statusSync) updateStatus(newIngressPoint []apiv1.LoadBalancerIngress) {
	...
	for _, ing := range ings {
		...
		if ingressSliceEqual(curIPs, newIngressPoint) {
			klog.V(3).InfoS("skipping update of internal (no change)", "namespace", ing.Namespace, "ingress", ing.Name)
			continue
		}
...
func runUpdate(ing *ingress.Ingress, status []apiv1.LoadBalancerIngress,
	...
	return func(wu pool.WorkUnit) (interface{}, error) {
		...
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("unexpected error searching internal %v/%v", ing.Namespace, ing.Name))
		}

		klog.InfoS("updating internal status", "namespace", currIng.Namespace, "ingress", currIng.Name, "currentValue", currIng.Status.LoadBalancer.Ingress, "newValue", status)
