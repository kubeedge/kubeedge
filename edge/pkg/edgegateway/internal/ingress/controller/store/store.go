...
import (
	...
	"github.com/kubeedge/kubeedge/edge/pkg/edgegateway/internal/ingress/controller/template"
...
// IngressFilterFunc decides if an internal should be omitted or not
...
func (s *k8sStore) syncSecret(key string) {
	...
	if err == nil {
		...
		// (like an update event from a change in internal)
		s.sendDummyEvent()
		return
	}
...
// getPemCertificate receives a secret, and creates a internal.SSLCert as return.
...
func New(
	...
	ingDeleteHandler := func(obj interface{}) {
		...
		if !ok {
			...
			if !ok {
				klog.Errorf("Tombstone contained object that is not an internal: %#v", obj)
				return
			}
		}
		...
		AddFunc: func(obj interface{}) {
			...
			if ings := store.secretIngressMap.Reference(key); len(ings) > 0 {
				...
				for _, ingKey := range ings {
					...
					if err != nil {
						klog.Errorf("could not find internal %v in local store", ingKey)
						continue
					}
					store.syncIngress(ing)
					store.syncSecrets(ing)
			...
			if !reflect.DeepEqual(old, cur) {
				...
				if ings := store.secretIngressMap.Reference(key); len(ings) > 0 {
					...
					for _, ingKey := range ings {
						ing, err := store.getIngress(ingKey)
						if err != nil {
							klog.ErrorS(err, "could not find internal in local store", "ingress", ingKey)
							continue
						}
						store.syncIngress(ing)
						store.syncSecrets(ing)
	...
	handleCfgMapEvent := func(key string, cfgMap *corev1.ConfigMap, eventName string) {
		...
		for _, ingKey := range ings {
			...
			if err != nil {
				klog.Errorf("could not find internal %v in local store: %v", key, err)
				continue
			}
...
// updateSecretIngressMap takes an internal and updates all Secret objects it
...
// internal with the local store and file system.
...
// IngressLister makes a Store that lists internal.
type IngressLister struct {
	cache.Store
}

// ByKey returns the internal matching key in the local internal Store.
...
// IngressWithAnnotationsLister makes a Store that lists internal rules with annotations already parsed
type IngressWithAnnotationsLister struct {
	cache.Store
}

// ByKey returns the internal with annotations matching key in the local store or an error
...
// SSLCertTracker holds a store of referenced Secrets in internal rules
...
// used to keep track of which data objects (Secrets) are used within internal
// objects.
...
func sortIngressSlice(ingresses []*ingress.Ingress) {
	...
	sort.SliceStable(ingresses, func(i, j int) bool {
		...
		if ir.Equal(&jr) {
			...
			klog.V(3).Infof("internal %v and %v have identical CreationTimestamp", in, jn)
			return in > jn
		}
		return ir.Before(&jr)
	})
...
// getIngress returns the internal matching key.
...
func (s *k8sStore) writeSSLSessionTicketKey(cmap *corev1.ConfigMap, fileName string) {
	...
	if ticketString != "" {
		...
		err = ioutil.WriteFile(fileName, decodedTicket, edgegateway.ReadWriteByUser)
		if err != nil {
