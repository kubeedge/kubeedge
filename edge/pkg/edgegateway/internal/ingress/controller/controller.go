...
import (
	...
	ngx_config "github.com/kubeedge/kubeedge/edge/pkg/edgegateway/internal/ingress/controller/config"
...
func (n *NginxController) syncIngress(interface{}) error {
	...
	if !n.IsDynamicConfigurationEnough(pcfg) {
		klog.Info("Configuration changes detected, backend reload required")
		...
		klog.Info("Backend successfully reloaded")
...
func (n *NginxController) getBackendServers(ingresses []*ingress.Ingress) ([]*ingress.Backend, []*ingress.Server) {
	...
	for _, ing := range ingresses {
		...
		for _, rule := range ing.Spec.Rules {
			...
			if rule.HTTP == nil &&
				host != defServerName {
				klog.V(3).Infof("internal %q does not contain any HTTP rule, using default backend", ingKey)
				continue
			}
			...
			if server.CertificateAuth.CAFileName == "" {
				...
				if server.CertificateAuth.Secret != "" && server.CertificateAuth.CAFileName == "" {
					klog.V(3).Infof("Secret %q has no 'ca.crt' key, mutual authentication disabled for internal %q",
						server.CertificateAuth.Secret, ingKey)
				}
			} else {
				klog.V(3).Infof("Server %q is already configured for mutual authentication (internal %q)",
			...
			if !n.store.GetBackendConfiguration().ProxySSLLocationOnly {
				if server.ProxySSL.CAFileName == "" {
					...
					if server.ProxySSL.Secret != "" && server.ProxySSL.CAFileName == "" {
						klog.V(3).Infof("Secret %q has no 'ca.crt' key, client cert authentication disabled for internal %q",
							server.ProxySSL.Secret, ingKey)
					}
				} else {
					klog.V(3).Infof("Server %q is already configured for client cert authentication (internal %q)",
						server.Hostname, ingKey)
				}
			}
			...
			for _, path := range rule.HTTP.Paths {
				...
				for _, loc := range server.Locations {
					...
					if !loc.IsDefBackend {
						klog.V(3).Infof("Location %q already configured for server %q with upstream %q (internal %q)",
							loc.Path, server.Hostname, loc.Backend, ingKey)
						break
					}

					klog.V(3).Infof("Replacing location %q for server %q with upstream %q to use upstream %q (internal %q)",
				...
				// new location
				if addLoc {
					klog.V(3).Infof("Adding location %q for server %q with upstream %q (internal %q)",
...
func (n *NginxController) getStreamServices(configmapName string, proto apiv1.Protocol) []ingress.L4Service {
	...
	for port, svcRef := range configmap.Data {
		...
		if reserverdPorts.Has(externalPort) {
			klog.Warningf("Port %d cannot be used for %v stream services. It is reserved for the internal controller.", externalPort, proto)
			continue
		}
...
func (n *NginxController) createServers(data []*ingress.Ingress,
	...
	for _, ing := range data {
		...
		if anns.Canary.Enabled {
			klog.V(2).Infof("internal %v is marked as Canary, ignoring", ingKey)
			continue
		}
		...
		if ing.Spec.Backend != nil {
			...
			if backendUpstream, ok := upstreams[defUpstream]; ok {
				// use backend specified in internal as the default backend for all its rules
				un = backendUpstream.Name

				// special "catch all" case, internal with a backend but no rule
				...
				if defLoc.IsDefBackend && len(ing.Spec.Rules) == 0 {
					klog.V(2).Infof("internal %q defines a backend but no rule. Using it to configure the catch-all server %q", ingKey, defServerName)
					...
					klog.V(3).Infof("internal %q defines both a backend and rules. Using its backend as default upstream for all its rules.", ingKey)
				}
			}
		}
	...
	for _, ing := range data {
		...
		for _, rule := range ing.Spec.Rules {
			...
			if len(servers[host].Aliases) == 0 {
				...
				klog.Warningf("Aliases already configured for server %q, skipping (internal %q)", host, ingKey)
			}
			...
			if anns.ServerSnippet != "" {
				if servers[host].ServerSnippet == "" {
					...
					klog.Warningf("Server snippet already configured for server %q, skipping (internal %q)",
						host, ingKey)
				}
			}
			...
			if len(ing.Spec.TLS) == 0 {
				klog.V(3).Infof("internal %q does not contains a TLS section.", ingKey)
				continue
			}
...
func (n *NginxController) serviceEndpoints(svcKey, backendPort string) ([]ingress.Endpoint, error) {
	...
	// internal with an ExternalName Service and no port defined for that Service
...
func (n *NginxController) getServiceClusterEndpoint(svcKey string, backend *networking.IngressBackend) (endpoint ingress.Endpoint, err error) {
	...
	// if the Service port is referenced by name in the internal, lookup the
...
// Compares an internal of a potential alternative backend's rules with each existing server and finds matching host + path pairs.
	...
	if ing.Spec.Backend != nil {
		...
		if altUps == nil {
			...
			for _, loc := range servers[defServerName].Locations {
				...
				if altEqualsPri {
					klog.Warningf("alternative upstream %s in internal %s/%s is primary upstream in Other internal for location %s%s!",
