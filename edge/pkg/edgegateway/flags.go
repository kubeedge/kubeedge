...
The class of an internal object is set using the field IngressClassName in Kubernetes clusters version v1.18.0 or higher or the annotation "kubernetes.io/ingress.class" (deprecated).
If this parameter is not set, or set to the default value of "nginx", it will handle ingresses with either an empty or "nginx" class name.`)
		...
		publishSvc = flags.String("publish-service", "",
			`Service fronting the internal controller.
...
status of all internal objects it satisfies.`)
...
defined by the healthz-port parameter are forwarded internally to this path.`)
		...
		updateStatus = flags.Bool("update-status", true,
			`Update the load-balancer status of internal objects this controller satisfies.
Requires setting the publish-service parameter to a valid Service reference.`)
		...
		electionID = flags.String("election-id", "ingress-controller-leader",
			`Election id to use for internal status updates.`)

		updateStatusOnShutdown = flags.Bool("update-status-on-shutdown", true,
			`Update the load-balancer status of internal objects when the controller shuts down.
Requires the update-status parameter.`)

		useNodeInternalIP = flags.Bool("report-node-internal-ip-address", false,
			`Set the load-balancer status of internal objects to internal Node addresses instead of external.
Requires the update-status parameter.`)

		showVersion = flags.Bool("version", false,
			`Show release information about the NGINX internal controller and exit.`)
	...
	`Prefix of the internal annotations specific to the NGINX controller.`)
...
extension for this to succeed.`)
		...
		publishStatusAddress = flags.String("publish-status-address", "",
			`Customized address to set as the load-balancer status of internal objects this controller satisfies.
Requires the update-status parameter.`)
...
https://blog.maxmind.com/2019/12/18/significant-changes-to-accessing-and-using-geolite2-databases`)
	...
	pflag.VisitAll(func(flag *pflag.Flag) {
		klog.V(2).Infof("FLAG", flag.Name, flag.Value)
	})
	...
	if *statusUpdateInterval < 5 {
		klog.Warningf("The defined time to update the internal status too low (%v seconds). Adjusting to 5 seconds", *statusUpdateInterval)
	...
	if *ingressClass != "" {
		klog.Infof("Watching for internal", "class", *ingressClass)

		if *ingressClass != class.DefaultClass {
			klog.Warningf("Only Ingresses with class %q will be processed by this internal controller", *ingressClass)
		} else {
			klog.Warning("Ingresses with an empty class will also be processed by this internal controller")
		}
	...
	if nginx.MaxmindLicenseKey != "" && nginx.MaxmindEditionIDs != "" {
		...
		klog.Infof("downloading maxmind GeoIP2 databases")
		if err := nginx.DownloadGeoLite2DB(); err != nil {
			klog.Error(err, "unexpected error downloading GeoIP2 database")
		}
