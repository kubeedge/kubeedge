...
func (nodeUpgradeJobActionHandler) checkItems(ctx context.Context, specser SpecSerializer) ActionResponse {
...
func (h *nodeUpgradeJobActionHandler) backup(_ctx context.Context, _specser SpecSerializer) ActionResponse {
	resp := new(nodeUpgradeJobActionResponse)
	cmd := execs.NewCommand("keadm backup edge")
...
func (nodeUpgradeJobActionHandler) upgrade(_ctx context.Context, specser SpecSerializer) ActionResponse {
	resp := new(nodeUpgradeJobActionResponse)
	spec, ok := specser.GetSpec().(*operationsv1alpha2.NodeUpgradeJobSpec)
	if !ok {
		resp.err = fmt.Errorf("failed to conv spec to NodeUpgradeJobSpec, actual type %T", specser.GetSpec())
	...
	if getter := spec.ImageDigestGetter; getter != nil {
		...
		case runtime.GOARCH == "arm64" && getter.ARM64 != "":
			cmdline.WriteString(" --image-digest " + getter.ARM64)
		case runtime.GOARCH == "amd64" && getter.AMD64 != "":
			cmdline.WriteString(" --image-digest " + getter.AMD64)
		}
	}
...
func (nodeUpgradeJobActionHandler) rollback(_ctx context.Context, _specser SpecSerializer) ActionResponse {
	...
	cmd := execs.NewCommand("keadm rollback edge")
	resp.err = cmd.Exec()
	return resp
}
