...
// forwarded to rotateChan (never to the drainable reconnectChan), back-to-back
// rotations coalesce into one pending signal without blocking, and the
// goroutine exits when done is closed.
func TestIfRotationDoneSendsToRotateChan(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		case <-time.After(time.Second):
			t.Fatalf("ifRotationDone did not consume certManager.Done (send %d)", i+1)
		}
	}
	...
	select {
	case <-eh.rotateChan:
		t.Fatal("back-to-back rotations must coalesce into a single pending signal")
	default:
	}
	select {
	case <-eh.reconnectChan:
	...
	case <-exited:
	case <-time.After(time.Second):
		t.Fatal("ifRotationDone did not exit after done was closed")
	}
}
