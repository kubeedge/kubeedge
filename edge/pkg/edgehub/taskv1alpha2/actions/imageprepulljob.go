...
func newImagePrePullJobRunner() *ActionRunner {
	...
	runner.addAction(string(operationsv1alpha2.ImagePrePullJobActionCheck), funcs.checkItems)
	runner.addAction(string(operationsv1alpha2.ImagePrePullJobActionPull), funcs.pullImages)
	return runner
}
...
func (imagePrePullJobFuncs) checkItems(_ctx context.Context, specser SpecSerializer) ActionResponse {
...
func (imagePrePullJobFuncs) pullImages(ctx context.Context, specser SpecSerializer) ActionResponse {
	resp := new(imagePrePullJobActionResponse)
	spec, ok := specser.GetSpec().(*operationsv1alpha2.ImagePrePullJobSpec)
	if !ok {
		resp.err = fmt.Errorf("failed to conv spec to ImagePrePullJobSpec, actual type %T", specser.GetSpec())
	...
	if imgsec := spec.ImagePrePullTemplate.ImageSecret; imgsec != "" {
		...
		if len(named) != 2 {
			resp.err = fmt.Errorf("pull secret format is not correct")
			return resp
		}
		client := metaclient.New()
		secret, err := client.Secrets(named[0]).Get(named[1])
		if err != nil {
			resp.err = fmt.Errorf("failed to get secret %s/%s, err: %v", named[0], named[1], err)
			return resp
		}
		if err = json.Unmarshal(secret.Data[corev1.DockerConfigJsonKey], &authcfg); err != nil {
			resp.err = fmt.Errorf("failed to unmarshal secret %s/%s to auth config, err: %v",
				named[0], named[1], err)
			return resp
		}
	}
	...
	for _, image := range spec.ImagePrePullTemplate.Images {
		...
		err := imgrt.PullImage(ctx, image, &authcfg, nil)
		if err != nil {
			if resp.err == nil {
				resp.err = errors.New(pullImageFailureMessage)
			}
			st.State = metav1.ConditionFalse
			st.Reason = err.Error()
		} else {
			st.State = metav1.ConditionTrue
		}
...
func (f imagePrePullJobFuncs) reportActionStatus(jobname, nodename, action string, resp ActionResponse) {
	...
	if resp, ok := resp.(*imagePrePullJobActionResponse); ok &&
		...
		if err != nil {
			f.logger.Error(err, "failed to marshal image status")
		} else {
			extend = string(bff)
		}
	}
