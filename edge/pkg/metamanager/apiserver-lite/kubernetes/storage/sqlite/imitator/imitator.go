...
import (
	...
	"github.com/kubeedge/kubeedge/edge/pkg/metamanager/apiserver-lite/kubernetes/storage/sqlite/imitator/watchhook"
...
func (s *imitator)InsertOrUpdateObj(ctx context.Context,obj runtime.Object) error{
	key ,err := apiserverlite.KeyFuncObj(obj)
	utilruntime.Must(err)
	...
	s.lock.Unlock()
	klog.V(4).Infof("[apiserver-lite]successfully insert or update obj:%v",key)
	return nil
...
func (s *imitator)Get(ctx context.Context, key string)(Resp,error){
	var resp Resp
	s.lock.RLock()
	results ,err:= v2.RawMetaByGVRNN(apiserverlite.ParseKey(key))
...
func (s *imitator)Event(msg *model.Message) []watch.Event{
	klog.V(4).Infof("[apiserver-lite] get a message from metamanager: %+v",msg)
