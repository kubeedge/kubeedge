...
func TestGetServiceAccountToken(t *testing.T) {
	...
	sendMock := newMockSend()
	sat := &serviceAccountToken{send: sendMock}
	...
	sendMock.On("SendSync", mock.Anything).Return(responseMsg, nil).Once()
	...
	sendMock.ExpectedCalls = nil
	sendMock.On("SendSync", mock.Anything).Return(responseMsg, fmt.Errorf("send error")).Once()
