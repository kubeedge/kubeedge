...
func TestFactoryListCacheMiss(t *testing.T) {
	...
	patches.ApplyFunc(handlers.ListResource, func(_ interface{}, _ interface{}, _ *handlers.RequestScope, _ bool, _ time.Duration) http.Handler {
	...
	cachedHandler, exists := factory.handlers["list"]
	assert.True(t, exists)
	assert.NotNil(t, cachedHandler)
}
...
func TestFactoryDeleteCacheMiss(t *testing.T) {
	...
	patches.ApplyFunc(handlers.DeleteResource, func(_ interface{}, _ bool, _ *handlers.RequestScope, _ interface{}) http.Handler {
...
func TestParseTimeout(t *testing.T) {
	...
	timeout = parseTimeout("invalid")
	assert.Equal(t, 34*time.Second, timeout)
...
func TestLimitedReadBody(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/test", strings.NewReader(content))
	data, err := limitedReadBody(req, 100)
	assert.NoError(t, err)
	...
	req = httptest.NewRequest("POST", "/test", strings.NewReader(content))
	data, err = limitedReadBody(req, 0)
	assert.NoError(t, err)
	assert.Equal(t, content, string(data))
}
