		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				if app == nil {
					...
					if !ok {
						t.Errorf("Application not stored")
					} else if stored != app {
						t.Errorf("Stored application doesn't match returned application")
					}
				}
			}
		})
...
func TestApplyWithRejectedStatus(t *testing.T) {
	...
	app, _ := agent.Generate(ctx, "get", metav1.GetOptions{}, nil)
...
func TestListWatchApplications(t *testing.T) {
	...
	watchApp, _ := a.Generate(ctx, "watch", metav1.ListOptions{Watch: true}, nil)
	nonWatchApp, _ := a.Generate(ctx, "get", metav1.GetOptions{}, nil)
