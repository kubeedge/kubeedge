	...
	for {
		...
		case model.UnholdPodUpgrade:
			...
			if updates, exists := e.heldPodUpdates[key]; exists {
				klog.V(1).Infof("Unholding pod upgrade: %s", key)
				for _, update := range updates {
					rawUpdateChan <- update
				}
				delete(e.heldPodUpdates, key)
			} else {
				klog.V(1).Infof("No held updates found for pod %s", key)
			}
...
func (e *edged) updateHeldUpgradeCondition(pod *v1.Pod) error {
	...
	klog.V(5).Infof("Generated patch for Pod %s/%s, length: %d, content: %s", pod.Namespace, pod.Name, len(content), string(content))
