...
and the full file path is k8s.io/kubernetes/pkg/volume/csi/csi_client.go
and make some modifications including:
1. add controllerV1ClientCreator
2. add CreateVolume function.
3. add DeleteVolume function.
4. add ControllerPublishVolume function.
5. add ControllerUnpublishVolume function.
6. add ControllerGetCapabilities function.
*/
...
// This is the default implementation for the nodeV1ClientCreator, used in
// newCsiDriverClient.
func newV0NodeClient(addr csiAddr) (nodeClient csipbv1.NodeClient, closer io.Closer, err error) {
	var conn *grpc.ClientConn
	conn, err = newGrpcConn(addr)
	if err != nil {
		return nil, nil, err
...
func asCSIAccessModeV0(am api.PersistentVolumeAccessMode) csipbv1.VolumeCapability_AccessMode_Mode {
	switch am {
	case api.ReadWriteOnce:
		return csipbv1.VolumeCapability_AccessMode_SINGLE_NODE_WRITER
	case api.ReadOnlyMany:
