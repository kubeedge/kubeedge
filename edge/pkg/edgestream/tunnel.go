...
func (s *TunnelSession) serveMetricsConnection(m *stream.Message) error {
	metricCon := &stream.EdgedMetricsConnection{
		ReadChan: make(chan *stream.Message, 128),
	}
	if err := json.Unmarshal(m.Data, metricCon); err != nil {
		klog.Errorf("unmarshal connector data error %v", err)
		return err
	}

	s.AddLocalConnection(m.ConnectID, metricCon)
	return metricCon.Serve(s.Tunnel)
}
