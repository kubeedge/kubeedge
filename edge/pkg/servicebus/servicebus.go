...
func buildBasicHandler(timeout time.Duration) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if targetURL, _ := dao.GetUrlsByKey(sReq.TargetURL); targetURL == nil {
			sResp.Code = http.StatusBadRequest
			sResp.Msg = fmt.Sprintf("url %v is not allowed and please make a rule for this url in the cloud", sReq.TargetURL)
			w.Write(marshalResult(sResp))
			return
		}
		msg := beehiveModel.NewMessage("").BuildRouter("servicebus", modules.UserGroup,
