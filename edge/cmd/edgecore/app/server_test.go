...
func TestBuildServiceBusTLSOptions(t *testing.T) {
	tests := []struct {
		...
		{
			// cert-only: TLS is intended; buildTLSConfig will fail because the
			// key is missing, which is the correct behavior (no silent HTTP fallback).
			name: "TLSCertFile set but TLSPrivateKeyFile empty (cert-only)",
			sb: &v1alpha2.ServiceBus{
				TLSCertFile:       "/etc/kubeedge/certs/servicebus-server.crt",
				TLSPrivateKeyFile: "",
			},
		...
		{
			// key-only: TLS is intended; buildTLSConfig will fail because the
			// cert is missing, which is the correct behavior (no silent HTTP fallback).
			name: "TLSPrivateKeyFile set but TLSCertFile empty (key-only)",
			sb: &v1alpha2.ServiceBus{
				TLSCertFile:       "",
				TLSPrivateKeyFile: "/etc/kubeedge/certs/servicebus-server.key",
			},
