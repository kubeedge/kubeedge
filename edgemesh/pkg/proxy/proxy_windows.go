...
func pollCallback(conn net.Conn) {
	value, ok := serve.Load(uniqueNameFromConn(conn))
	if !ok {
		return
	}
...
func uniqueNameFromConn(conn net.Conn) string {
...
func addServer(svcName string, ports []int32) {
	...
	for _, port := range ports {
		...
		conn, err := ln.Accept()
		serve.Store(uniqueNameFromConn(conn), server)
		if err != nil {
			klog.Error("[L4 Proxy] %s", err)
			return
		}
		err = epoll.EpollCtrlAdd(conn)
		if err != nil {
			klog.Error("[L4 Proxy] %s", err)
			return
		}
