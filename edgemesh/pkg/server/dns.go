...
//isAQurey judge if the dns pkg is a Qurey process
...
func parseNameServer()([]net.IP){
	...
	if err != nil {
		glog.Errorf("the resolv.conf has sone error : %s")
	}
	defer file.Close()
...
// converQueRsp conversion the dns head to a response for one query
func (h *dnsHeader)converQueRsp(isRsp bool){
	if isRsp {
		h.flags |= dnsQr
	}else {
		h.flags |= dnsQr
