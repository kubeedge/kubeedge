...
func TestReconcileClusterObjectSync(t *testing.T) {
	...
	defer func() {
		sendToEdge = originalSendToEdge
		gcOrphanedClusterObjectSyncFunc = originalGcFunc
		buildEdgeControllerMessageFunc = originalBuildEdgeControllerMessageFunc
		getNodeNameFunc = originalGetNodeNameFunc
		getObjectUIDFunc = originalGetObjectUIDFunc
		compareResourceVersionFunc = originalCompareResourceVersionFunc
	}()
...
func TestGcOrphanedClusterObjectSyncFunction(t *testing.T) {
	...
	defer func() {
		sendToEdge = originalSendToEdge
		buildEdgeControllerMessageFunc = originalBuildEdgeControllerMessageFunc
		deleteClusterObjectSyncFunc = originalDeleteFunc
		getNodeNameFunc = originalGetNodeNameFunc
		getObjectUIDFunc = originalGetObjectUIDFunc
		gcOrphanedClusterObjectSyncFunc = originalGcFunc
	}()
...
func TestGcOrphanedClusterObjectSyncFuncIntegration(t *testing.T) {
	originalGcFunc := gcOrphanedClusterObjectSyncFunc
	originalBuildMsgFunc := buildEdgeControllerMessageFunc
	originalSendFunc := sendToEdge
	originalDeleteFunc := deleteClusterObjectSyncFunc
	originalGetNodeNameFunc := getNodeNameFunc
	originalGetObjectUIDFunc := getObjectUIDFunc
	...
	defer func() {
		gcOrphanedClusterObjectSyncFunc = originalGcFunc
		buildEdgeControllerMessageFunc = originalBuildMsgFunc
		sendToEdge = originalSendFunc
		deleteClusterObjectSyncFunc = originalDeleteFunc
		getNodeNameFunc = originalGetNodeNameFunc
		getObjectUIDFunc = originalGetObjectUIDFunc
	}()
