...
func (dc *DownstreamController) deviceAdded(device *v1beta1.Device) {
	...
	_, err := dc.ensureDeviceStatusForDevice(device)
	if err != nil {
...
// ensureDeviceStatusForDevice ensures device status CRD is created for the device
func (dc *DownstreamController) ensureDeviceStatusForDevice(device *v1beta1.Device) (*v1beta1.DeviceStatus, error) {
	deviceId := util.GetResourceID(device.Namespace, device.Name)
	_, exists := dc.deviceStatusManager.DeviceStatus.Load(deviceId)
	if exists {
		return nil, nil
	}
	...
	_, err := dc.crdClient.DevicesV1beta1().DeviceStatuses(device.Namespace).Create(context.Background(), deviceStatus, metav1.CreateOptions{})
