...
// List lists all DeviceModels in the indexer.
func (s *deviceModelLister) List(selector labels.Selector) (ret []*v1alpha1.DeviceModel, err error) {
...
// Get retrieves the DeviceModel from the indexer for a given namespace and name.
func (s deviceModelNamespaceLister) Get(name string) (*v1alpha1.DeviceModel, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("devicemodel"), name)
	}
	return obj.(*v1alpha1.DeviceModel), nil
}
