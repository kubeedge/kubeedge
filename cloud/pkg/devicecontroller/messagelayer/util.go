...
func BuildResource(nodeID, resourceType, resourceID string) (resource string, err error) {
	...
	resource = fmt.Sprintf("%s%s%s%s%s", constants.ResNode, constants.ResourceSep, nodeID, constants.ResourceSep, resourceType)
	if resourceID != "" {
...
func GetDeviceID(resource string) (string, error) {
	...
	if len(res) >= constants.ResourceDeviceIDIndex+1 && res[constants.ResourceDeviceIndex] == constants.ResDevice {
