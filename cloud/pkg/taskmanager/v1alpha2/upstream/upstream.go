...
func updateNodeJobTaskStatus(res taskmsg.Resource,
	...
	if action.IsFinal() {
		if nodetask.Status() == operationsv1alpha2.NodeTaskStatusInProgress {
			nodetask.SetStatus(operationsv1alpha2.NodeTaskStatusSuccessful)
		}
		...
		// The next action here will definitely not be nil. The cloud actively transfers
		// to the next action, which can reduce the number of reports from the edge.
		nodetask.SetAction(action.Next(upmsg.Succ))
	}
