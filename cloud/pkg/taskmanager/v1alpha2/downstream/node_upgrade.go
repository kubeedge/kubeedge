...
func (h *NodeUpgradeJobHandler) CanDownstreamStatus(obj any, _isInInitialList bool) bool {
	...
	return job.Status.State == operationsv1alpha2.JobStateInit ||
		job.Status.State == operationsv1alpha2.JobStateInProgress
}
...
func (h *NodeUpgradeJobHandler) HandleNodeActionError(
	ctx context.Context,
	job wrap.NodeJob,
	errTask wrap.NodeJobTask,
	err error,
) {
	h.logger.Error(err, "failed to handle node action error",
		"job name", job.Name(), "node name", errTask.NodeName())
	...
	nodeTaskStatus, ok := errTask.GetObject().(*operationsv1alpha2.NodeUpgradeJobNodeTaskStatus)
	if !ok {
		h.logger.Error(nil, "failed to convert task to NodeUpgradeJobNodeTaskStatus",
			"invalid type", reflect.TypeOf(errTask.GetObject()))
		return
	}
		...
		Callback: func(err error) {
			if err != nil {
				h.logger.Error(err, "failed to update NodeUpgradeJob node task status",
					"job name", nodeUpgradeJob.Name, "node name", nodeTaskStatus.NodeName)
			}
		},
	}
