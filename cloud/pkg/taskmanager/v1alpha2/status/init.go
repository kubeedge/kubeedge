...
func tryUpdateImagePrePullJobStatus(
	...
	opts UpdateStatusOptions[operationsv1alpha2.ImagePrePullNodeTaskStatus],
) error {
	job, err := cli.OperationsV1alpha2().ImagePrePullJobs().
		Get(ctx, opts.JobName, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("faield to get image prepull job %s, err: %v", opts.JobName, err)
	}
	for i := range job.Status.NodeStatus {
		status := &job.Status.NodeStatus[i]
		if status.NodeName == opts.NodeTaskStatus.NodeName {
			job.Status.NodeStatus[i] = opts.NodeTaskStatus
			break
		}
	}
...
func tryUpdateNodeUpgradeJobStatus(
	ctx context.Context,
	cli crdcliset.Interface,
	opts UpdateStatusOptions[operationsv1alpha2.NodeUpgradeJobNodeTaskStatus],
) error {
	job, err := cli.OperationsV1alpha2().NodeUpgradeJobs().
		Get(ctx, opts.JobName, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("faield to get node upgrade job %s, err: %v", opts.JobName, err)
	}
	for i := range job.Status.NodeStatus {
		status := &job.Status.NodeStatus[i]
		if status.NodeName == opts.NodeTaskStatus.NodeName {
	...
	_, err = cli.OperationsV1alpha2().NodeUpgradeJobs().
		UpdateStatus(ctx, job, metav1.UpdateOptions{})
	if err != nil {
		return fmt.Errorf("failed to update node upgrade job %s status, err: %v", opts.JobName, err)
	}
	return nil
}
