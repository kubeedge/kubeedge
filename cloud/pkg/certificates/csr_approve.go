...
// Package certificates implements an automated approver for KubeEdge metaserver certificates.
...
func NewCSRApprovingController(client clientset.Interface, csrInformer certificatesinformers.CertificateSigningRequestInformer) *CSRApprovingController {
	cc := &CSRApprovingController{
		kubeClient: client,
...
func (cc *CSRApprovingController) Run(workers int, stopCh <-chan struct{}) {
	defer utilruntime.HandleCrash()
	defer cc.queue.ShutDown()

	klog.Info("Starting CSRApprovingController")
	defer klog.Info("Shutting down CSRApprovingController")

	if !cache.WaitForNamedCacheSync("CSRApprovingController", stopCh, cc.csrsSynced) {
		return
	}
...
func (cc *CSRApprovingController) worker() {
	for cc.processNextWorkItem() {
	}
}

// processNextWorkItem deals with one key off the queue.  It returns false when it's time to quit.
func (cc *CSRApprovingController) processNextWorkItem() bool {
...
func (cc *CSRApprovingController) enqueueCertificateRequest(obj interface{}) {
...
func (cc *CSRApprovingController) syncFunc(key string) error {
	startTime := time.Now()
	defer func() {
...
func recognizers() []csrRecognizer {
	recognizers := []csrRecognizer{
		{
			recognize:      isEdgeCoreServingCert,
			...
			successMessage: "Auto approving EdgeCore serving certificate.",
		},
	}
	return recognizers
}

func (cc *CSRApprovingController) handle(csr *capi.CertificateSigningRequest) error {
...
func isEdgeCoreServingCert(csr *capi.CertificateSigningRequest, x509cr *x509.CertificateRequest) bool {
