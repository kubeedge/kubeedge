...
func TestMutateOfflineMigration(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !response.Allowed {
				t.Error("Expected admission to be allowed, got false")
			}

			// Always expect a patch (based on implementation)
			if len(response.Patch) == 0 {
				t.Error("Expected patch to be generated, but got empty patch")
			}
			...
			if tc.checkUnreachableToleration {
				...
				if !foundUnreachableToleration {
					t.Error("Expected NodeUnreachable toleration not found")
				}
			}
		})
	}
...
func TestMutateOfflineMigrationErrorHandling(t *testing.T) {
	...
	if response.Allowed {
		t.Error("Expected admission to be not allowed for invalid input")
	}
