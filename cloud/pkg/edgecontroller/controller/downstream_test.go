...
func (tml *TestMessageLayer) Receive() (model.Message, error) {
	...
	return model.Message{}, fmt.Errorf("no messages available")
}
...
func TestNewDownstreamController(t *testing.T) {
	...
	k8sInformerFactory := informers.NewSharedInformerFactory(client, 0)
	...
	dc, err := NewDownstreamController(
		config,
		k8sInformerFactory,
		directKeInformer,
		crdInformerFactory,
		WithKubeClient(client),
		WithMessageLayer(messageLayer),
	)
