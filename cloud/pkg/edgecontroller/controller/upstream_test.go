...
// Helper function to create test messages for pod status
func createPodStatusMessage(id, podName, podNamespace string, podUID types.UID, status corev1.PodStatus, operation string) model.Message {
	...
	podStatusData, _ := json.Marshal(podStatus)
	nodeID := defaultNodeID
	resource := fmt.Sprintf("node/%s/%s/%s/%s", nodeID, podNamespace, model.ResourceTypePodStatus, podName)
...
func TestPodStatusErrorPaths(t *testing.T) {
	...
	msg1 := createPodStatusMessage(
		"test-pod-status-not-found",
		"non-existent-pod",
		podNamespace,
		podUID,
	...
	msg3 := createPodStatusMessage(
		"test-pod-status-terminating",
		podName,
		podNamespace,
		podUID,
		podStatus,
...
func TestUpdatePodStatus(t *testing.T) {
	...
	msg := createPodStatusMessage(
		"test-update-pod-status",
		podName,
		podNamespace,
		podUID,
		status,
