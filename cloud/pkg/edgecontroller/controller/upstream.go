...
func (uc *UpstreamController) patchNode() {
	for {
		...
		case msg := <-uc.patchNodeChan:
			...
			if !ok {
				klog.Warningf("message: %s process failure, get data failed with error: %s", msg.GetID(), err)
				continue
			}

			nodeByte, err := base64.URLEncoding.DecodeString(dataReq)
			if err != nil {
				klog.Warningf("message: %s process failure, decode content data failed with error: %s", msg.GetID(), err)
				continue
			}
			...
			if err != nil {
				klog.Errorf("message: %s process failure, patch node failed with error: %s, namespace: %s, name: %s", msg.GetID(), err, namespace, name)
			}
...
func (uc *UpstreamController) patchPod() {
	for {
		...
		case msg := <-uc.patchPodChan:
			...
			if err != nil {
				klog.Errorf("message: %s process failure, update pod failed with error: %s, namespace: %s, name: %s", msg.GetID(), err, namespace, name)
			}
			...
			resMsg := model.NewMessage(msg.GetID()).
				...
				BuildRouter(modules.EdgeControllerModuleName, constants.GroupResource, msg.GetResource(), model.ResponseOperation)
			if err = uc.messageLayer.Response(*resMsg); err != nil {
				klog.Errorf("Message: %s process failure, response failed with error: %s", msg.GetID(), err)
				continue
			}

			klog.V(4).Infof("message: %s, patch pod status successfully, namespace: %s, name: %s", msg.GetID(), namespace, name)
		}
	}
}
	...
	for {
		...
		case msg := <-uc.createLeaseChan:
			...
			case model.InsertOperation:
				...
				if err != nil {
					errLog := fmt.Sprintf("create lease %s failed, error: %v", name, err)
					klog.Error(errLog)
				}
			...
			case model.UpdateOperation:
				...
				if err != nil {
					errLog := fmt.Sprintf("update lease %s failed, error: %v", name, err)
					klog.Error(errLog)
				}
...
func (uc *UpstreamController) queryLease() {
	for {
		...
		case msg := <-uc.queryLeaseChan:
			...
			leaseResp, err := uc.kubeClient.CoordinationV1().Leases(namespace).Get(context.TODO(), name, metaV1.GetOptions{})
			if err != nil {
				errLog := fmt.Sprintf("query lease %s failed, error: %v", name, err)
				klog.Error(errLog)
			}

			resMsg := model.NewMessage(msg.GetID()).
				FillBody(&ObjectResp{Object: leaseResp, Err: err}).
				BuildRouter(modules.EdgeControllerModuleName, constants.GroupResource, msg.GetResource(), model.ResponseOperation)
			if err = uc.messageLayer.Response(*resMsg); err != nil {
				klog.Warningf("Response message: %s failed, response failed with error: %s", msg.GetID(), err)
				continue
