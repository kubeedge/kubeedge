...
import (
	...
	"github.com/kubeedge/kubeedge/cloud/pkg/csrapprover"
...
kubernetes controller which manages devices so that the device metadata/status date can be synced between edge and cloud.`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			if features.DefaultFeatureGate.Enabled(features.RequireAuthorization) {
				csrApprover := csrapprover.NewCSRApprover(client.GetKubeClient(), informers.GetInformersManager().GetKubeInformerFactory().Certificates().V1().CertificateSigningRequests())
