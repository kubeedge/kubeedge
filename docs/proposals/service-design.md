...
1. Since the edge environment can be some non-standard system to running some specific devices, we should not rely on the kernel network capability. In that way, we need to use userspace proxy to implement the data proxy on edge.
...
7. All edge nodes are in the same sub-network and they have the network reachability.
...
* Need to setup http_proxy in the container(can be done automatically) with the address of ServiceBus. Then the application can vist the service provider via `http://{service_name}/resource`. ServiceBus get the service name part from the request as a reverse proxy, then pick up one POD to redirect the request. The other option is to use environment variable or hosts files to describe the service address(which is kubeedge/servicebus address actually). In this way, it will be possible to support non-REST communication.
