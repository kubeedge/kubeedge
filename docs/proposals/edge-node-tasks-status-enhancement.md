...
Add a new node job controller for the each node job CRD. The execution steps of Reconcile(..) function in the node job controller:
1. If the node job CR is just created, initialize all matched nodes and set the job phase to "Init".
1. If the node job is already in the final phase, it means that Reconcile(..) function is triggered by updating the final phase, call return to interrupt the function.
1. Calculates the node job status through the node task status on each node.
1. Call kube-apiserver to update the node job status.
...
1. Create a controller for the node job in ControllerManager and complete the relevant logic of the controller.
