...
There are scenarios user needs to run a standalone Kubernetes cluster at edge to get full control and improve the offline scheduling capability. There are two scenarios user need to do that:
...
* User needs to deploy an edge environment with limited resources and offline running for most of the time

  In some IOT scenarios, user needs to deploy a full control edge environment and running offline.
