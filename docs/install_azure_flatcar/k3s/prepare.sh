...
scp -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa master_prepare.sh master_install.sh master_uninstall.sh values.yaml 10-containerd-net.conflist cloudcore-chart-1.18.tar.gz traefik-config.yaml traefik-ingress-route-tcp0.yaml traefik-ingress-route-tcp1.yaml traefik-ingress-route-tcp2.yaml traefik-ingress-route-tcp3.yaml traefik-ingress-route-tcp4.yaml nginx-onlyedge.yaml core@${MASTER_IP}:~/
