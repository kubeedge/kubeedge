/*
Copyright 2019 The Kubeedge Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package common

const (
	//KubeEdgeVersion sets the version of KubeEdge to be used
	KubeEdgeVersion = "kubeedge-version"

	//DockerVersion sets the version of Docker to be used
	DockerVersion = "docker-version"

	//KubernetesVersion sets the version of Kuberneted to be used
	KubernetesVersion = "kubernetes-version"

	//KubeConfig sets the path of kubeconfig , default is ""
	KubeConfig = "kube-config"

	//K8SAPIServerIPPort sets the IP:Port of Kubernetes api-server
	K8SAPIServerIPPort = "k8sserverip"

	//EdgeControllerIP sets the IP of KubeEdge edgecontroller cloud component
	EdgeControllerIP = "edgecontrollerip"

	//EdgeNodeID KubeEdge Node unique idenfitcation string
	EdgeNodeID = "edgenodeid"

	//EdgeProjectID set project id string
	EdgeProjectID = "edgeprojectid"

	//CertPath sets the path of the certificates generated by the KubeEdge Cloud component
	CertPath = "certPath"

	// VendorK8sPrefix
	VendorK8sPrefix = "v1.10.9-kubeedge-"

	// RuntimeType is default runtime type
	RuntimeType = "runtimetype"

	// KubeAPIServer sets the api-server
	KubeAPIServer = "kube-apiserver"

	// LoggerLevel sets the log level
	LoggerLevel = "logger-level"

	// WebSocketPort set websocket port
	WebSocketPort = "websocket-port"
	// QuicPort set quic port
	QuicPort = "quic-port"

	//Protocol set the connection protocol between edge and edgecontroller
	Protocol = "protocol"
)

const (
	//DefaultCertPath is the default certificate path in edge node
	DefaultCertPath = "/etc/kubeedge/certs"

	//DefaultKubeEdgeVersion is the current default version of KubeEdge
	DefaultKubeEdgeVersion = "0.3.0"

	//DefaultDockerVersion is the current default version of Docker
	DefaultDockerVersion = "18.06.0"

	//DefaultK8SVersion is the current default version of K8S
	DefaultK8SVersion = "1.14.1"

	// DefaultProjectID is default project id
	DefaultProjectID = "e632aba927ea4ac2b575ec1603d56f10"

	// DefaultKubeAPIServer set the default apiserver url
	DefaultKubeAPIServer = "http://localhost:8080"

	// DefaultLoggerLevel set the default logger level
	DefaultLoggerLevel = "INFO"

	// DefaultKubeConfig set the default kube config path
	DefaultKubeConfig = ""

	// DefaultWebSocketPort set default websocket port
	DefaultWebSocketPort = 10000

	// DefaultQuicPort set default quic port
	DefaultQuicPort = 10001

	// DefaultKeepAliveInterval is default keepalive Interval for cloudhub
	DefaultKeepAliveInterval = 30

	// DefaultWriteTimeout is default write timeout for cloudhub
	DefaultWriteTimeout = 30

	// DefaultNodeLimit is default node limit for cloudhub
	DefaultNodeLimit = 10

	// DefaultRuntimeType set the default runtime type
	DefaultRuntimeType = "docker"

	// DefaultProtocol set default protocol
	DefaultProtocol = "websocket"
)
