...
func TestInstallMQTT(t *testing.T) {
	...
	p4 := gomonkey.ApplyFunc(getOSVendorName, func() (string, error) {
		return "CentOS", nil
	})
	defer p4.Reset()
	...
	p4 = gomonkey.ApplyFunc(getOSVendorName, func() (string, error) {
		return openEulerVendorName, nil
	})
...
func TestIsK8SComponentInstalled(t *testing.T) {
	...
	err := rpmOS.IsK8SComponentInstalled(kubeConfig, master)
	assert.NoError(t, err)
}
