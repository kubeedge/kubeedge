...
func (mo *mockOSTypeInstaller) IsKubeEdgeProcessRunning(name string) (bool, error) {
	return mo.runningProcess, mo.runningErr
}
...
func TestSetKubeEdgeVersion(t *testing.T) {
	...
	mockVersion, _ := semver.Make("1.5.0")
...
func TestGetCurrentVersion(t *testing.T) {
	...
	patches.ApplyFunc(GetCurrentVersion, func(version string) (string, error) {
		if version == "v1.8.0" || version == "1.8.0" {
			return "v1.8.0", nil
		}
		return "v1.20.0", nil
	})

	version, err := GetCurrentVersion("v1.8.0")
	assert.NoError(t, err)
	assert.Equal(t, "v1.8.0", version)
...
func TestGetOSInterface(t *testing.T) {
	...
	_, isMockOS := osInterface.(*mockOSTypeInstaller)
...
func TestCommand(t *testing.T) {
	...
	patches.ApplyMethod(mockCmd, "Run", func(*exec.Cmd) error {
		return nil
	})

	patches.ApplyMethod(mockCmd, "Output", func(*exec.Cmd) ([]byte, error) {
	...
	cmd = NewCommand("failing command")
	err = cmd.Exec()
	assert.Error(t, err)
...
func TestInstallKubeEdge(t *testing.T) {
	...
	mockOS.installErr = errors.New("install error")
	err = common.InstallKubeEdge(options)
	assert.Error(t, err)
...
func TestIsK8SComponentInstalledMethod(t *testing.T) {
	...
	mockOS.k8sInstalledErr = errors.New("not installed")
	err = common.IsK8SComponentInstalled("kubelet", "v1.20.0")
	assert.Error(t, err)
...
func TestKillKubeEdgeBinary(t *testing.T) {
	...
	mockOS.killProcessErr = errors.New("kill error")
	err = common.KillKubeEdgeBinary("edgecore")
	assert.Error(t, err)
