...
func TestGetNameFormStatus(t *testing.T) {
	testCases := []struct {
		...
		// Test case 4
		{
			s:	"name1[0].variable1.value2 = value1",
			expectResult: 1,
		},
	}
...
func TestParseAndSetArrayValue(t *testing.T) {
	...
	err = ParseAndSetArrayValue(&testStruct, "ArrayField[10]", 5)
	if err == nil {
...
func TestSetArrayValue(t *testing.T) {
	...
	err = SetArrayValue(&testStruct, "ArrayField", 1, "incorrect")
	if err == nil {
		t.Error("Expected an error for setting incorrect value type, but got nil")
	}
}
...
func TestParseFieldPath1(t *testing.T) {
	testCases := []struct {
		...
		// Test case 2: Valid field path with negative index
		{
			fieldPath:       "ArrayField[-1]",
			expectedPath:    "ArrayField",
			expectedIndex:   -1,
		...
		// Test case 3: Invalid field path (no index)
		{
		...
		// Test case 4: Invalid field path (invalid index)
		{
			fieldPath:       "ArrayField[abc]",
			expectedPath:    "",
			expectedIndex:   -1,
...
func TestSetVariableValue_InvalidPath(t *testing.T) {
    
    config := &Config{}

    
    err := SetVariableValue(config, "Invalid.Path", 100)
    if err == nil {
