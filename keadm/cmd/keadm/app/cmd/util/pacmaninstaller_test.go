...
func TestInstallMQTT(t *testing.T) {
	pacmanOS := PacmanOS{}

	cmd := &Command{}

	p1 := gomonkey.ApplyFunc(NewCommand, func(command string) *Command {
		expectedCmd := "ps aux |awk '/mosquitto/ {print $11}' | awk '/mosquit/ {print}'"
	...
	p3 := gomonkey.ApplyMethod(reflect.TypeOf(cmd), "GetStdOut", func(*Command) string {
		return "mosquitto"
	})
	defer p3.Reset()

	err := pacmanOS.InstallMQTT()
	assert.NoError(t, err)

	p1.Reset()

	p4 := gomonkey.ApplyFunc(NewCommand, func(command string) *Command {
		if command == "ps aux |awk '/mosquitto/ {print $11}' | awk '/mosquit/ {print}'" {
			p5 := gomonkey.ApplyMethod(reflect.TypeOf(cmd), "GetStdOut", func(*Command) string {
				return ""
			})
			defer p5.Reset()
			return cmd
		...
		if command == "pacman -Syy --noconfirm mosquitto" {
			p6 := gomonkey.ApplyMethod(reflect.TypeOf(cmd), "GetStdOut", func(*Command) string {
				return "mosquitto installed successfully"
			})
			defer p6.Reset()
			return cmd
		}
	...
	p7 := gomonkey.ApplyMethod(reflect.TypeOf(cmd), "Exec", func(*Command) error {
