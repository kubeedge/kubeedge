...
// This function returns false, if the file already exists or an error an error occurred.
func Occupy() (bool, error) {
	if IsOccupied() {
		return false, nil
	}
	if _, err := os.Create(idempotencyRecord); err != nil {
		return false, fmt.Errorf("failed to create idempotency_record file, err: %v", err)
	}
	return true, nil
}
