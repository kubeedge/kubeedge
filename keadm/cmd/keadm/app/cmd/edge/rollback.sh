...
if [ $? -ne 0 ]; then
    logger_Error "unenable installer auto start failed"
    logger_Error "backup failed"
    write_idempotency_record ${HANDLE_UPGRADE_FAILED}
    exit 1
fi

# replace old edgecore with origin edgecore
cp ${BACKUP_PATH}/edgecore /usr/local/bin/edgecore
cp ${BACKUP_PATH}/edgecore.yaml ${EDGECORE_CONFIG_PATH}
cp ${BACKUP_PATH}/edgecore.db ${DB_PATH}

start_service ${EDGE_CORE_SERVICE_FILE}
if [ $? -eq 0 ]; then
    logger_Info "Complete starting old edge_core process"
else
    write_idempotency_record ${HANDLE_UPGRADE_FAILED}
    logger_Error "Failed to start old edge_core process, please check"
    exit 1
fi

write_idempotency_record ${HANDLE_UPGRADE_FINISHED}
logger_Info "Congratulations, restore edgecore successfully."
