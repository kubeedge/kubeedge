...
func batchJoinNodes(config *Config) error {
	...
	for _, node := range config.Nodes {
		...
		go func(n Node) {
			...
			if err := CopyKeadmToNode(&n, arch); err != nil {
				klog.Errorf("Processing node %s  CopyKeadmToNode error", n.IP)
				return
			}
			// Execute the keadm join command
			if err := executeJoinCommand(&n, &config.KeadmParams, arch); err != nil {
...
func CopyKeadmToNode(node *Node, arch string) error {
...
func executeJoinCommand(node *Node, params *KeadmParams, arch string) error {
