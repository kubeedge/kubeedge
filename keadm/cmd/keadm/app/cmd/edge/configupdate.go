...
func (executor *configUpdateExecutor) configUpdate(opts ConfigUpdateOptions) error {
	edgeConfigure := v1alpha2.NewDefaultEdgeCoreConfig()
	err := edgeCoreConfig.Parse(opts.Config)
	if err != nil {
		return fmt.Errorf("failed to get edgecore config file %s: %v", opts.Config, err)
	}
