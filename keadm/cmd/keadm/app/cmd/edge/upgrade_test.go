...
func TestUpgradeExecutorUpgrade(t *testing.T) {
	...
	patches.ApplyFunc(runEdgeCore, func(_withMqtt bool) error {
		return nil
	})
...
func TestGetEdgeCoreBinary(t *testing.T) {
	...
	patches.ApplyFunc(util.NewContainerRuntime, func(endpoint, cgroupDriver string,
		...
		checked++
		return &util.CRIRuntime{}, nil
	})
	patches.ApplyMethodFunc(reflect.TypeOf(&util.CRIRuntime{}), "PullImage",
		func(image string, _authConfig *runtimeapi.AuthConfig, _sandboxConfig *runtimeapi.PodSandboxConfig) error {
			assert.Equal(t, opts.Image+":"+opts.ToVersion, image)
			checked++
			return nil
		})
	patches.ApplyMethodFunc(reflect.TypeOf(&util.CRIRuntime{}), "CopyResources",
		func(edgeImage string, files map[string]string) error {
