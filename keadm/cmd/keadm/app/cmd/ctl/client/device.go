...
func (deviceRequest *DeviceRequest) GetFakeDevices(ctx context.Context) (*v1beta1.DeviceList, error) {
	versionedClient, err := VersionedKubeClient()
	if err != nil {
		return nil, err
	}

	device0, err := versionedClient.DevicesV1beta1().Devices(deviceRequest.Namespace).Get(ctx, "beta1-device", metaV1.GetOptions{})
	if err != nil {
		return nil, err
	}

	device1, err := versionedClient.DevicesV1beta1().Devices(deviceRequest.Namespace).Get(ctx, "beta1-device-1", metaV1.GetOptions{})
	if err != nil {
		return nil, err
	}

	deviceList := &v1beta1.DeviceList{
		Items: []v1beta1.Device{*device0, *device1},
	}
...
func (deviceRequest *DeviceRequest) UpdateDevice(ctx context.Context, device *v1beta1.Device) (*rest.Result, error) {
	...
	// _, err = versionedClient.DevicesV1beta1().Devices(deviceRequest.Namespace).Update(ctx, device, metaV1.UpdateOptions{})
	// if err != nil {
	// 	return nil, err
	// }
