...
func TestVerificationParameters(t *testing.T) {
	...
	fileExistsPatch := gomonkey.ApplyFunc(util.FileExists, func(path string) bool {
	...
	fileExistsConfigPatch := gomonkey.ApplyFunc(util.FileExists, func(path string) bool {
		return path == opts.Config
	})
	...
	fileExistsAllPatch := gomonkey.ApplyFunc(util.FileExists, func(path string) bool {
		return true
	})
...
func TestCollectSystemData(t *testing.T) {
	...
	mkdirPatch := gomonkey.ApplyFunc(os.Mkdir, func(path string, perm os.FileMode) error {
		assert.Equal("/tmp/system", path)
		assert.Equal(os.ModePerm, perm)
		return nil
	})
	defer mkdirPatch.Reset()

	execShellPatch := gomonkey.ApplyFunc(ExecuteShell, func(cmdStr string, tmpPath string) error {
		return nil
	})
	...
	mkdirSuccessPatch := gomonkey.ApplyFunc(os.Mkdir, func(path string, perm os.FileMode) error {
		return nil
	})
	...
	execShellSuccessPatch := gomonkey.ApplyFunc(ExecuteShell, func(cmdStr string, tmpPath string) error {
		return nil
	})
...
func TestCollectEdgecoreData(t *testing.T) {
	...
	mkdirPatch := gomonkey.ApplyFunc(os.Mkdir, func(path string, perm os.FileMode) error {
		assert.Equal("/tmp/edgecore", path)
		assert.Equal(os.ModePerm, perm)
		return nil
	})
	defer mkdirPatch.Reset()
...
func TestExecuteCollect(t *testing.T) {
	...
	verifyErrorPatch := gomonkey.ApplyFunc(VerificationParameters, func(collectOptions *common.CollectOptions) error {
		return fmt.Errorf("parameter verification failed")
	})
	...
	collectSystemErrorPatch := gomonkey.ApplyFunc(collectSystemData, func(tmpPath string) error {
		return fmt.Errorf("system data collection failed")
	})
