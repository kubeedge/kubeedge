...
func addJoinOtherFlags(cmd *cobra.Command, joinOptions *options.JoinOptions) {
	...
	cmd.Flags().Lookup(options.CertPath).NoOptDefVal = options.DefaultCertPath
...
func newJoinOptions() *options.JoinOptions {
	return &options.JoinOptions{CertPath: options.DefaultCertPath}
}

func checkFlags(f *pflag.Flag) {
	if f.Name == options.CertPath {
