...
function cleanup() {
  ...
  # The namespace cleanup timeout may occur if pods is not cleared
...
function start_kubeedge() {
  ...
  # Join with CRI-O as the container runtime.
  # --remote-runtime-endpoint points to the CRI-O unix socket.
  sudo -E CHECK_EDGECORE_ENVIRONMENT="false" /usr/local/bin/keadm join \
    --token=$TOKEN \
