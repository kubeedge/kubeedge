...
GO_LDFLAGS='$(hack/make-rules/version.sh)'

function cleanup() {
  sudo pkill edgecore || true
  helm uninstall cloudcore -n kubeege && kubectl delete ns kubeedge  || true
...
function start_kubeedge() {
  ...
  cd $HELM_DIR && helm upgrade --install --wait --timeout 30s cloudcore ./cloudcore --namespace kubeedge --create-namespace -f ./cloudcore/values.yaml --set cloudCore.modules.cloudHub.advertiseAddress[0]=${MASTER_IP} --set cloudCore.image.tag=${IMAGE_TAG}
  export MASTER_IP=`kubectl get nodes -o jsonpath={.items[0].status.addresses[0].address}`
