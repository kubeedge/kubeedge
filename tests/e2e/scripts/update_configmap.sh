...
EDGE_PATH=${SRC_DIR}/tests/e2e/assets/02-edgeconfigmap.yaml
CLOUD_PATH=${SRC_DIR}/tests/e2e/assets/01-configmap.yaml
EDGESITE_PATH=${SRC_DIR}/tests/e2e/assets/03-edgesiteconfigmap.yaml
...
create_edgesite_config() {
    ...
    echo "file found !!!!!!!!!!!!!"
    sed -i "s|namespace: .*|namespace: default|g" ${EDGESITE_PATH}
    sed -i "s|name: edgesite-configmap.*|name: ${configmapName}|g" ${EDGESITE_PATH}
    sed -i "s|node-id: .*|node-id: ${nodename}|g" ${EDGESITE_PATH}
    sed -i "s|node-name: .*|node-name: ${nodename}|g" ${EDGESITE_PATH}
    sed -i "s|hostname-override: .*|hostname-override: ${nodename}|g" ${EDGESITE_PATH}
}
