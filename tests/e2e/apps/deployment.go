	...
	ginkgo.Context("Application Deployment and Deletion using Deployment Spec", func() {
		...
		ginkgo.It("E2E_APP_DEPLOYMENT_4:  Create a deployment with liveness probe and verify that it works", func() {
			...
			for _, pod := range podList.Items {
				...
				gomega.Expect(probe.TimeoutSeconds).To(gomega.Equal(int32(1)))
				gomega.Expect(probe.InitialDelaySeconds).To(gomega.Equal(int32(10)))
				gomega.Expect(probe.PeriodSeconds).To(gomega.Equal(int32(15)))
			}
		})
	})
