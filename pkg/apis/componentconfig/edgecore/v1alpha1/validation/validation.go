...
func ValidateModuleEdgeHubVault(h v1alpha1.EdgeHubVault) field.ErrorList {
	...
	if h.Enable {
		...
		if h.Address == "" {
			allErrs = append(allErrs, field.Invalid(field.NewPath("Vault"), h.Address,
				"Vault must not be empty"))
		}

		if _, err := url.ParseRequestURI(h.Address); err != nil {
			allErrs = append(allErrs, field.Invalid(field.NewPath("Vault"), h.Address,
		...
		if h.PKI == "" {
			allErrs = append(allErrs, field.Invalid(field.NewPath("PKI"), h.PKI,
				"PKI must not be empty"))
		}
	}
	return allErrs
}
