...
var linkByNameFunc = func(name string) (netlink.Link, error) {
	return nil, fmt.Errorf("not implemented")
}

var linkAddFunc = func(link netlink.Link) error {
	return fmt.Errorf("not implemented")
}

var linkDelFunc = func(link netlink.Link) error {
	return fmt.Errorf("not implemented")
}

var addrListFunc = func(link netlink.Link, family int) ([]netlink.Addr, error) {
	return nil, fmt.Errorf("not implemented")
}

var addrAddFunc = func(link netlink.Link, addr *netlink.Addr) error {
	return fmt.Errorf("not implemented")
}

func (d *DummyDeviceManager) LinkByName(name string) (netlink.Link, error) {
	return linkByNameFunc(name)
}

func (d *DummyDeviceManager) LinkAdd(link netlink.Link) error {
	return linkAddFunc(link)
}

func (d *DummyDeviceManager) LinkDel(link netlink.Link) error {
	return linkDelFunc(link)
}

func (d *DummyDeviceManager) AddrList(link netlink.Link, family int) ([]netlink.Addr, error) {
	return addrListFunc(link, family)
}

func (d *DummyDeviceManager) AddrAdd(link netlink.Link, addr *netlink.Addr) error {
	return addrAddFunc(link, addr)
}
...
func TestEnsureDummyDevice(t *testing.T) {
	...
	t.Run("Device already exists", func(t *testing.T) {
		originalLinkByName := linkByNameFunc
		defer func() { linkByNameFunc = originalLinkByName }()
	...
	t.Run("Device doesn't exist and created successfully", func(t *testing.T) {
		originalLinkByName := linkByNameFunc
		originalLinkAdd := linkAddFunc
		defer func() {
			linkByNameFunc = originalLinkByName
			linkAddFunc = originalLinkAdd
		}()
		...
		exists, err := manager.EnsureDummyDevice("dummy0")
		assert.False(t, exists, "Expected device not to exist initially")
		assert.NoError(t, err, "Expected no error on creation")
	})
...
func TestDeleteDummyDevice(t *testing.T) {
	...
	t.Run("Device exists and deleted successfully", func(t *testing.T) {
		originalLinkByName := linkByNameFunc
		originalLinkDel := linkDelFunc
		defer func() {
			linkByNameFunc = originalLinkByName
			linkDelFunc = originalLinkDel
		}()
...
func TestListBindAddress(t *testing.T) {
	...
	t.Run("Device exists and has addresses", func(t *testing.T) {
		originalLinkByName := linkByNameFunc
		originalAddrList := addrListFunc
		defer func() {
			linkByNameFunc = originalLinkByName
			addrListFunc = originalAddrList
		}()
...
func TestEnsureAddressBind(t *testing.T) {
	...
	t.Run("Device exists and address bound successfully", func(t *testing.T) {
		originalLinkByName := linkByNameFunc
		originalAddrAdd := addrAddFunc
		defer func() {
			linkByNameFunc = originalLinkByName
			addrAddFunc = originalAddrAdd
		}()
