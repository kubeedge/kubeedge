...
func TestExecConnection_Serve(t *testing.T) {
	...
	// Use a custom RoundTripperFunc to avoid reflection issues
	patches := gomonkey.NewPatches()
	defer patches.Reset()

	patches.ApplyFunc(http.NewRequest, func(method, urlStr string, body io.Reader) (*http.Request, error) {
		return &http.Request{}, nil
	})

	// Explicitly use *tls.Config as the parameter type for spdy.NewRoundTripper
	patches.ApplyFunc(spdy.NewRoundTripper, func(_ *tls.Config) (http.RoundTripper, error) {
		return mockRoundTripper, nil
	})

	// Function to send stop signal after a delay
	go func() {
		time.Sleep(100 * time.Millisecond)
		edgedExecConn.Stop <- struct{}{}
	}()
	...
	// Check if RemoveConnect message was sent
	// Give deferred function enough time to execute
